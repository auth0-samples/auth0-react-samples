...
Next, open `src/App.js` where the Auth0 SDK client is initialized and specify the audience value from the configuration in the `audience` property:

```js
const auth0 = await createAuth0Client({
  domain: config.domain,
  client_id: config.clientId,
  audience: config.audience // NEW - specify the audience value here
});
```
...
To provide a mechanism for calling the API, create a new component in the `views` folder called `ExternalApi.js` and populate it with the following content:
...
class ExternalApi extends Component {
  ...
  callApi = async () => {
    ...
    try {
      // Get the token from the Auth0 client
      const token = await auth0.getTokenSilently();

      // Call the API, specifying the token as the bearer token inside
  ...
  render() {
    ...
    return (
      <>
        <div>
          <h1>External API</h1>
          <button onClick={this.callApi}>Ping API</button>
        </div>
...
The new route should have a `path` value of `/external-api`:

```jsx
// src/App.js

// First import the component at the top of the file:
import ExternalApi from "./components/ExternalApi";

// Then modify the route table by adding a new private route
<BrowserRouter>
  <Navbar
    auth0={auth0}
  ...
  <Switch>
    ...
    {/* NEW - add a new private route pointing to the External API component */}
    <PrivateRoute path="/external-api" auth0={auth0} component={ExternalApi} />
  </Switch>
</BrowserRouter>;
```
...

{
  isAuthenticated && (
    <>
      ...
      {/* NEW - include a link to the /external-api page */}
